var workTimeLength = 60;
var pauseTimeLength = 10;
var interval;
var workFlag; //if workFlag is true, pause timer will start after work timer is done

window.onload = function () {
	let timerLength =
		workTimeLength < 10 ? '0' + workTimeLength + ':00' : workTimeLength + ':00';
...
function startTimer(defaultTimer) {
	...
	interval = setInterval(function () {
		...
		let minutesString = Math.floor(
			(defaultTimer * 60 - differenceTime / 1000) / 60
		);
...
function validateInputTime(input, id) {
	if (input === '')
		//if html input doesn't send anything because of wrong value ex: --5 send default value
